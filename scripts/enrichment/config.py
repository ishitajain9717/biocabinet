...
class EnrichmentConfig:
    ...
    pathway_path:     Optional[Path]        # combined pathway embeddings .pt (None ⇒ no pathway features)
    ...
    # ---- inference ----
    pairs_path:       Optional[Path] = None # TSV of candidate ENSP pairs (None ⇒ skip inference)
...
def collect_config_from_user() -> EnrichmentConfig:
    ...
    esm_path     = _ask_path("Path to ESM-2 embeddings .pt", default="data/esm2_embeddings_SHS27k.pt")
    ...
    out_dir_raw = _ask("Output directory (will be created)", default="runs/enrichment_run")
    ...
    if use_defaults:
        ...
        cfg = EnrichmentConfig(
            ...
            pathway_path=pathway_path,
            out_dir=out_dir,
            epochs=_ask_int("epochs", 100),
            batch_size=_ask_int("train batch_size", 512),
            lr=_ask_float("learning rate", 1e-3),
            weight_decay=_ask_float("weight_decay", 1e-4),
            early_stop=_ask_int("early-stop patience (epochs)", 20),
            split_method=_ask_choice("split method", ["random", "bfs", "dfs"], "random"),
            test_size=_ask_float("val fraction", 0.2),
            seed=_ask_int("seed", 1),
            device=_ask_choice("device", ["auto", "cuda", "mps", "cpu"], "auto"),
