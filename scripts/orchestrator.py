    ...
    if state.get("enrichment_ran"):
        ...
        if state.get("enrichment_error"):
            parts.append(
                ...
                f"Last enrichment message:\n  {state.get('enrichment_summary') or '(none)'}"
            )
        else:
            parts.append(
...
def build_graph(memory, parent_thread_id: str) -> StateGraph:
    ...
    def graph_node_run_bulk(state: OrchestratorState) -> dict:
        ...
        summary, error, child_state = _run_child(
            build_bulk, memory, parent_thread_id, "bulk",
            return_state=True,
        )
        # Stash deg_pairs_path so the enrichment auto-chain can pick it up.
        deg_pairs = (child_state or {}).get("deg_pairs_path") if child_state else None
        return {
            "child_summary":  summary,
            "child_error":    error,
            "bulk_deg_pairs": deg_pairs,
        }
    ...
    def graph_node_run_enrichment(state: OrchestratorState) -> dict:
        ...
        summary, error = _run_child(
            build_enrichment, memory, parent_thread_id, "enrichment",
    ...
    def graph_node_offer_rag_chat(state: OrchestratorState) -> dict:
        ...
        Pipeline context (DEG path, n_deg, bulk summary) is forwarded so
        the answerer can focus retrieval on the actual experiment's genes.
        """
        print("\n" + "=" * 60)
        ...
        # Build context the chat loop can reference
        pipeline_ctx = {
            "deg_sig_path":      None,          # filled below if available
            "n_deg_significant": None,
            "child_summary":     state.get("child_summary") or "",
        }
        # Try to recover deg_sig_path from bulk_deg_pairs directory
        bp = state.get("bulk_deg_pairs")
        if bp:
            sig_candidate = Path(bp).parent / "deseq2_significant.tsv"
            if sig_candidate.exists():
                pipeline_ctx["deg_sig_path"] = str(sig_candidate)

        from scripts.rag.graph import build_rag_chat_graph
        _run_child(
            build_rag_chat_graph, memory, parent_thread_id, "rag_chat",
            initial_state={"pipeline_context": pipeline_ctx, "should_quit": False},
        )
        return {"rag_chat_ran": True}
...
def run(thread_id: str | None = None, db_path: Path = DEFAULT_DB) -> dict:
    ...
    print(f"    to resume:     python3 -m scripts.orchestrator --thread-id {thread_id}\n")
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--thread-id",
        default=None,
        help="Resume an existing run by its thread_id (printed at start of previous run).",
    )
    p.add_argument(
        "--db",
